...
class BadgeSamplingAdapter:
    def __init__(self, new_badge_sampling_class):

        self.new_badge_sampling_class = new_badge_sampling_class()
        
    def get_grad_embedding(self, probs, feat):
        # Redirect to use the new method on the handler
        return self.badge_sampling.handler.get_grad_embedding(self.badge_sampling.model, probs, feat)
    
    def enable_dropout(self):
        # Redirect to use the new method on the handler    
        return self.badge_sampling.handler.enable_dropout(self.badge_sampling.model)

    def query(self, leave_pbar=True, cur_epoch=None):
        # Basically translating the old interface's query to the new one

        training_config = self.badge_sampling.training_config
        query_size = 10  # Example fixed size, should be dynamically set based on old interface's needs
        device = self.badge_sampling.device

        return self.badge_sampling.query(self.badge_sampling.model, self.badge_sampling.handler, self.badge_sampling.dataset_handler, training_config, query_size, device)

#################################################

class NewBadgeSampling(StrategyNewInterface):
    def __init__(self, model : nn.Module, handler: ModelHandler, dataset_handler: DataSetHandler, training_config, device, *arg, **kwargs):
        # num_box decay rate
        # self.delta = 0.99
        # self.k = 3
        self.model = model
    ...
    @staticmethod
    def query(model : nn.Module, handler: ModelHandler, dataset_handler: DataSetHandler,training_config: Dict, query_size: int, device, *args, **kwargs) -> Tuple[List, List]:
    ...
    # def query(self, leave_pbar=True, cur_epoch=None):
        ...
        #----------------------------------
        unlabeled_loader = dataset_handler.get_unlabeled_loader()
        labeled_loader = dataset_handler.get_labeled_loader()
        #----------------------------------
        ...
        handler.get_grad_embedding(model, probs, feat)
        #----------------------------------
